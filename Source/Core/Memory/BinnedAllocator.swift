...
public final class BinnedAllocator: RawAllocator {
    ...
    private init() {
        ...
        // 16..256 step 16
        var s = 16
        while s <= 256 { table.append(s); s += 16 }
        // 288..512 step 32
        s = 288
        while s <= 512 { table.append(s); s += 32 }
        // 576..4096 step 64
        s = 576
        while s <= 4096 { table.append(s); s += 64 }
        self.classes = table
    ...
    private func spinLock(_ lock: ManagedAtomic<Int>) {
        while true {
            if lock.compareExchange(expected: 0, desired: 1, ordering: .acquiringAndReleasing).exchanged {
                return
            }
            // pause
            #if os(Windows)
    ...
    private func pushFree(_ cs: ClassState, _ p: UnsafeMutableRawPointer) {
        ...
        while true {
            ...
            if cs.freeHead.compareExchange(expected: cur, desired: newHead, ordering: .acquiringAndReleasing).exchanged {
    ...
    private func popFree(_ cs: ClassState) -> UnsafeMutableRawPointer? {
        while true {
            ...
            if cs.freeHead.compareExchange(expected: head, desired: desired, ordering: .acquiringAndReleasing).exchanged {
    ...
    private func markFreedIfEnabled(_ cs: ClassState, ptr: UnsafeMutableRawPointer) {
        ...
        if var words = blk.bitmapWords {
            ...
            if (words[w] & b) != 0 {
                ...
                updateListsOnFree(cs: cs, blockIndex: idx, binsFreeAfter: newFree, total: blk.binsTotal)
                cs.blocks[idx] = blk
            } else {
                cs.blocks[idx] = blk
            }
    ...
    private func grow(_ cs: ClassState) -> Bool {
        if !cs.growLock.compareExchange(expected: 0, desired: 1, ordering: .acquiringAndReleasing).exchanged {
        ...
        do {
            ...
            let block = Block(region: region, base: base, size: blkSize, binSize: cs.binSize, binsTotal: bins)
